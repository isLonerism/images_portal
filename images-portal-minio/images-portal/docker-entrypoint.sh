...
cd $(dirname "$0")
envsubst < data/.minio.sys/buckets/BUCKET_NAME/lifecycle.xml > data/.minio.sys/buckets/BUCKET_NAME/lifecycle.xml
...
do
	...
	envsubst < users/$user/identity.json > users/$user/identity.json
...
do
	envsubst < policies/$policy/policy.json > policies/$policy/policy.json
done
cd - > /dev/null
